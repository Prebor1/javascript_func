...
createOtherDivForSelect(htmlDropdown.parentElement) //az egyébhez (a hideBasedOnSelected() elé kell)
...
//11.21. orai function
    ...
    const szulo = dropdownList.parentElement; //a "parentElement" itt a <div>, masneven a szulo
    const kartya = szulo.querySelectorAll(".card");

    for (const elem of kartya){
...
function hideBasedOnCheckbox(checkbox){
    ...
    const parent = checkbox.parentElement; //szulot elkerjuk

    if (checkbox.checked){ // ha a checkbox be van checkelve...
        makeVisibleBasedOnId(parent, "ferfi") //a makevisible a ferfi, az elso a sorban
        makeInvisibleBasedOnid(parent, "no") //es a makevisible a no, a masodik
    }
    else { //ha nincs becheckelve...
        makeVisibleBasedOnId(parent, "no") //akkor itt az elso a makevisible no,
        makeInvisibleBasedOnid(parent, "ferfi") //ea a masodik makeinvisible ferfi
    }

    //A changeCheckbox(e) nelkul csak egyszer mukodik a checkbox valtoztatas, es akkkor is
    //csak akkor hogyha a html-ben ott van a "checked"
